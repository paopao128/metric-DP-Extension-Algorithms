...
for city_idx = 3:3 % length(city_list)
    ...
    load(node_file, 'col_longitude_orig', 'col_latitude_orig', 'col_osmid');
    NR_LOC = length(col_longitude_orig);
...
%% ------------------------ Start running the simulation here ------------------------------
    for test_idx = 1:1:NR_TEST                                              % This for loop repeats the experiments for NR_TEST times
        ...
        for epsilon_idx = 1:1:EPSILON_MAX                                   % This for loop indicates the different epsilon values
            ...
            loss_matrix_selected_ori=loss_matrix_selected;
            ...
            % corner_weights_selected_ori(i,j) is the bilinear weight of anchor j for real loc i.
            % Summing over all real locs gives the effective density each anchor represents.
            ...
            for epsilon_idx_1 = 1:1:NR_EPSILON_INTERVAL-1                   % Discretize epsilon_1 and try it one by one
                ...
                [z_aipo_instance_43160, loss_aipo_instance_43160(epsilon_idx, test_idx, epsilon_idx_1)] = logconv_interp(z_anchor_instance, corner_weights_selected, loss_matrix_selected_ori);
                ...
                for interp_id=1:interp_times
                    ...
                    % Log-convex interpolation with density-weighted prior
                    [z_extension, loss_aipo_instance(epsilon_idx, test_idx, epsilon_idx_1)] = logconv_interp(z_anchor_instance, corner_weights_selected, loss_matrix_selected, current_prior);
                    ...
                    loss_interp_all(epsilon_idx, interp_id, epsilon_idx_1) = loss_aipo_instance(epsilon_idx, test_idx, epsilon_idx_1);
        ...
        best_losses_all_tests(test_idx, :, :) = min(loss_interp_all, [], 3);
...
fprintf('extension_interpolation [%s] time: %s\n', city, strjoin(parts_t, ' & '));
fid = fopen('results_time.txt', 'a');
fprintf(fid, 'extension_interpolation [%s]: %s\n', city, strjoin(parts_t, ' & '));
fclose(fid);
