...
%% ------------------------ Start running the simulation here ------------------------------
    for test_idx = 1:1:NR_TEST                                              % This for loop repeats the experiments for NR_TEST times
        ...
        for epsilon_idx = 1:1:EPSILON_MAX                                   % This for loop indicates the different epsilon values
            ...
            loss_LP(epsilon_idx, test_idx) = 999999999;
            ...
            for epsilon_idx_1 = 1:1:NR_EPSILON_INTERVAL-1                   % Discretize epsilon_1 and try it one by one
                ...
                for interp_id=1:interp_times
                    ...
                    % LP extension with density-weighted prior
                    [z_extension,loss_LP_instance(epsilon_idx, test_idx, epsilon_idx_1)] = lp_extension_cellwise(cornerPoints, z_anchor_instance, allPoints, 0.5, loss_matrix_selected, current_prior);
                    ...
                    loss_LP_times(interp_id)=loss_LP_instance(epsilon_idx, test_idx, epsilon_idx_1);
                    loss_LP_all(epsilon_idx, interp_id, epsilon_idx_1) = loss_LP_instance(epsilon_idx, test_idx, epsilon_idx_1);
                ...
                % (epsilon_1, epsilon_2)
                if loss_LP_instance(epsilon_idx, test_idx, epsilon_idx_1) < loss_LP(epsilon_idx, test_idx)
                    loss_LP(epsilon_idx, test_idx) = loss_LP_instance(epsilon_idx, test_idx, epsilon_idx_1);
                    z_opt_LP = z_extension;
                end
            end
        ...
        best_losses_all_tests(test_idx, :, :) = min(loss_LP_all, [], 3);
