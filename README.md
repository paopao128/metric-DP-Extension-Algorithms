...
├── main_PANDA_exp.m                  # PAnDA baseline (anchor-based approximation)
│
├── — Appendix Experiments —
├── main_1norm_appendix.m             # ℓ₁ norm results (Appendix G)
├── main_ablation_budget_appendix.m   # Ablation: epsilon budget split (Appendix)
...
├── main_extension_draft.m            # Single extension round, log-convex
...
├── interpolation_extension_2.m       # Workspace script: simpler 2-fold refinement
...
│
├── — Analysis —
├── analyze_cost_approx_vs_real.m     # Formal analysis: c_approx vs cost_real across layers (10 trials)
├── discussion_cost_analysis.tex      # LaTeX appendix section for the above analysis
├── analysis_fig1_distribution.png    # Figure: value distribution comparison (log scale)
├── analysis_fig2_boxplot.png         # Figure: per-trial mean cost boxplot
├── analysis_fig3_scale.png           # Figure: scale ratio and anchor counts
├── analysis_fig4_scatter.png         # Figure: entry-level scatter, c_approx vs cost_real
│
...
│
├── PAnDA code/                       # PAnDA baseline implementation
├── datasets/                         # Road-network node/edge CSVs
│   ├── rome/nodes.csv, edges.csv
│   ├── nyc/nodes.csv, edges.csv
│   └── london/nodes.csv, edges.csv
...
## Analysis: c_approx vs cost_real

`analyze_cost_approx_vs_real.m` documents a structural gap between the approximated cost used in the seed LP and the true cost evaluated at each interpolation layer:

- **57× scale difference**: `c_approx` aggregates distances from all N=43,160 nodes (city-wide), while `cost_real` is computed from local grid points (tens to hundreds of meters).
- **Near-zero correlation** (r ≈ −0.11): the relative cost ordering between anchor–obfuscation pairs is essentially unrelated between the two matrices.

The LaTeX write-up is in `discussion_cost_analysis.tex` (insert after `\appendix`).  
